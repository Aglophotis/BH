...
void StatsDisplay::OnDraw()
{
	...
	if (!IsMinimized())
	{
		...
		Texthook::Draw(column1,
			...
			6,
			Red,
			string("ÿc4") + BH::menu->GetStringOrDefault("stat.absorp", "Absorpition") + string(": ÿc1%dÿc0/ÿc1%d%c ÿc3%dÿc0/ÿc3%d%c ÿc9%dÿc0/ÿc9%d%c ÿc8%dÿc0"),
			fAbsorb,
			fAbsorbPct,
			'%',
			cAbsorb,
		...
		Texthook::Draw(column1,
			...
			6,
			Gold,
			BH::menu->GetStringOrDefault("stat.elem_mastery", "Elemental Mastery") + string(":ÿc1 %d%%ÿc3 %d%%ÿc9 %d%%ÿc2 %d%%ÿc8"),
			fMastery,
			cMastery,
			lMastery,
			pMastery);
		...
		Texthook::Draw(column1,
			...
			6,
			Gold,
			BH::menu->GetStringOrDefault("stat.elem_pierce", "Elemental Pierce") + string(":ÿc1 %d%%ÿc3 %d%%ÿc9 %d%%ÿc2 %d%%ÿc8"),
			fPierce,
			cPierce,
			lPierce,
			pPierce);
